...
# PA 4 - Revised
military <- read_xlsx(here("gov_spending_per_capita.xlsx"),
...
rescale_01 <- function(vec) {
  ...
  if(length(vec) <= 1){
    stop('Please provide a vector with more than one element')
  }
  # (x - min) / (max - min)
...
# Lab 8, Q2
pivot_table <- function(df, col_var, row_var, fill = 0){
  df |>
